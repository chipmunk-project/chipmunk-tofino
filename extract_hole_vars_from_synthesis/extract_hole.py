...
def is_constant_set_def(line):
    search_result = re.search('int\[' + '\d+' + '\] constant_vector =', line)
...
def is_hole_assign(line):
    search_result = re.search('int' +' \w+ ' + '= \d+',line)
    if search_result == None:
       return 0
    else:
       return 1
...
def modify_salu_config(hole_list):
    ...
    # i.e ['heavy_hitters_canonicalizer_equivalent_0_raw_stateless_alu_6_7_salu_config_1_2', '1']
    #     to ['heavy_hitters_canonicalizer_equivalent_0_raw_stateless_alu_6_7_salu_config_1_2', '1', 'state_and_packet.state_group_2 be modified']
    for i in range(len(hole_list)):
        opt_find_result =  hole_list[i][0].find('salu_config')
        if opt_find_result != -1: 
            if hole_list[i][1] == '1':
                modified_state_var = 'state_and_packet.state_group_' + hole_list[i][0][-1]
                ...
                hole_list[i].append('stateful_alu in stage ' + stage_num + 'pipeline ' + alu_num + ' is not active')
    return hole_list

def set_phv_output_candidate(hole_name, num_of_stateful_group, num_of_group_member):
    ...
    for i in range(num_of_stateful_group):
        for j in range(num_of_group_member):
            stateful_var_member = 'old_state_group_' + stage_num + '_' + str(i) + '.state_' + str(j)
...
def modify_output_mux_phv(hole_list):
    ...
    for i in range(len(hole_list)):
        ...
        if salu_find_result != -1:
            num_of_stateful_group = max(num_of_stateful_group, int(hole_list[i][0][-1]) + 1)
    ...
    for i in range(len(hole_list)): 
        ...
        if output_mux_phv_find_result != -1:
            phv_output_candidate = set_phv_output_candidate(hole_list[i][0], num_of_stateful_group ,num_of_group_members)
...
def modify_mux_ctrl(hole_list, total_phv_per_stage):
    ...
    for i in range(len(hole_list)):
       ...
       if mux_ctrl_find_result != -1 and phv_find_result == -1:
          mux_output_candidate = set_mux_output_candidate(hole_list[i][0], total_phv_per_stage)
...
def modify_output_mux_global(hole_list):
    ...
    for i in range(len(hole_list)):
        output_mux_global_find_result = hole_list[i][0].find('output_mux_global')
...
def modify_Opt_in_pair(hole_list):
    for i in range(len(hole_list)):
        ...
        if Opt_0_find_result == -1:
            if hole_list[i][1] == '0':
                hole_list[i].append('return the value of state_0 in branch when True True for 2 if conds')
            else:
                hole_list[i].append('return the value 0 in branch when True True for 2 if conds')
        elif Opt_1_find_result == -1:
            if hole_list[i][1] == '0':
                hole_list[i].append('return the value of state_1 in branch when True True for 2 if conds')
            else:
                hole_list[i].append('return the value 0 in branch when True True for 2 if conds')
        elif Opt_2_find_result == -1:
            if hole_list[i][1] == '0':
                hole_list[i].append('return the value of state_0 in branch when True False for 2 if conds')
            else:
                hole_list[i].append('return the value 0 in branch when True False for 2 if conds')
        elif Opt_3_find_result == -1:
            if hole_list[i][1] == '0':
                hole_list[i].append('return the value of state_1 in branch when True False for 2 if conds')
            else:
                hole_list[i].append('return the value 0 in branch when True False for 2 if conds')
        elif Opt_4_find_result == -1:
            if hole_list[i][1] == '0':
                hole_list[i].append('return the value of state_0 in branch when if cond is false but else-if cond is true')
            else:
                hole_list[i].append('return the value 0 in branch when if cond is false but else-if cond is true')
        elif Opt_5_find_result == -1:
            if hole_list[i][1] == '0':
                hole_list[i].append('return the value of state_1 in branch when if cond is false but else-if cond is true')
            else:
                hole_list[i].append('return the value 0 in branch when if cond is false but else-if cond is true')
        elif Opt_6_find_result == -1:
            if hole_list[i][1] == '0':
                hole_list[i].append('return the value of state_0 in branch when if cond is false but else-if cond is false')
            else:
                hole_list[i].append('return the value 0 in branch when if cond is false but else-if cond is false')
        else:
            assert Opt_7_find_result == -1
            if hole_list[i][1] == '0':
                hole_list[i].append('return the value of state_1 in branch when if cond is false and else-if cond is false')
            else:
                hole_list[i].append('return the value 0 in branch when if cond is false but else-if cond is false')
    return hole_list
...
def modify_Opt_in_if_else_raw(hole_list):
    for i in range(len(hole_list)):
        ...
        if Opt_0_find_result == -1:
            ...
            if hole_list[i][1] == '0':
                hole_list[i].append('return the value of state_0 in branch when if cond is true')
            else:
                hole_list[i].append('return the value 0 in branch when if cond is true')
        else:
            assert Opt_2_find_result == -1
            if hole_list[i][1] == '0':
                hole_list[i].append('return the value of state_0 in branch when if cond is false')
            else:
                hole_list[i].append('return the value 0 in branch when if cond is false')            
    return hole_list
...
def modify_Opt_in_nested_ifs(hole_list):
    for i in range(len(hole_list)):
        ...
        if Opt_0_find_result == -1:
            if hole_list[i][1] == '0':
                hole_list[i].append('return the value of state_0 in first if cond')
            else:
                hole_list[i].append('return the value 0 in first if cond')
        elif Opt_1_find_result == -1:
            if hole_list[i][1] == '0':
                hole_list[i].append('return the value of state_0 in second if cond')
            else:
            ...
            if hole_list[i][1] == '0':
                ...
                hole_list[i].append('return the value 0 in branch in branch when True True for 2 if conds')
        elif Opt_3_find_result == -1:
            if hole_list[i][1] == '0':
                hole_list[i].append('return the value of state_0 in branch when True False for 2 if conds')
            else:
                hole_list[i].append('return the value 0 in branch when when True False for 2 if conds')
        elif Opt_4_find_result == -1:
            if hole_list[i][1] == '0':
...
def modify_Opt(hole_list, file_name):
    #TODO: deal with Opt
    stateful_alu_list = \
    ['pair','if_else_raw', 'pred_raw', 'nested_ifs', 'sub' , 'raw']
